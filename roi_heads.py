...
from torchvision.models.detection.roi_heads import det_utils, keypointrcnn_inference, keypointrcnn_loss, maskrcnn_inference
...
def fastrcnn_loss_focal(class_logits, box_regression, labels, regression_targets):
    ...
    classification_loss = focal_loss_multiclass(class_logits, labels, gamma=2.0)
...
def maskrcnn_loss_slitnet(mask_logits, proposals, gt_masks, gt_labels, mask_matched_idxs, power: float = 2.0):
    """
    Drop-in replacement for torchvision's maskrcnn_loss that uses
    """
    ...
    mask_logits_gt = mask_logits[idx, labels]  # [N,H,W]

    return hausdorff_dice_loss_from_logits_slitnet_tf_equiv(mask_logits_gt, mask_targets, power=power)

class SLITRoIHeads(nn.Module):
    __annotations__ = {
    ...
    def forward(
        ...
        if self.training:
            ...
            loss_classifier, loss_box_reg = fastrcnn_loss_focal(class_logits, box_regression, labels, regression_targets)
